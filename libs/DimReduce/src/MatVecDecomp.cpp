...
SVD::SVD(Matrix<double,DENSE>& A, 
	 ...
	 bool full_matrices/*=false*/) : S(A.rank()){
  ...
  if(A.isTransposed() ){
    throw NotImplementedError("SVD of transposed matrix");
  }
...
LUD::LUD(const Matrix <double,DENSE>& A) 
  : IPIV(A.rank()), LU(A), flag_(A.isTransposed()), sym_(false){
  dgetrf(A.nrows(), A.ncols(), LU.arr(), A.ld(), IPIV.arr() );
}

LUD::LUD(const Matrix <double,SYM>& A) 
  : IPIV(A.rank()), flag_(A.upper()), sym_(true){
  //copy matrix from A to LU
  LU.reallocate( A.nrows(),A.nrows() );
...
EIGS_AR::EIGS_AR(Matrix<double,SYM>& A, int nev, 
  ...
  : quiet_(quiet), evecs(A.nrows(),nev), evals(nev){
