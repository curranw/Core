...
MountainCar3DScaled::MountainCar3DScaled() : IDomain(){
    throttleFactor = 1.0;
    max_position = 0.6;
    min_position = -1.2;
    max_velocity = 0.07;
...
void MountainCar3DScaled::init()
{
    ...
    position_xpy = position_x + position_y;
    velocity_xpy = velocity_x + velocity_y;
    position_xty = position_x * position_y;
    velocity_xty = velocity_x * velocity_y;
}

vector<double> MountainCar3DScaled::get_state()
{
    vector<double> state;
    ...
    state.push_back(position_xpy);
    state.push_back(velocity_xpy);
    state.push_back(position_xty);
    state.push_back(velocity_xty);
    return state;
}
void MountainCar3DScaled::step(int action)
{
...
bool MountainCar3DScaled::end_of_episode()
{
    return (position_x >= target_position && position_y >= target_position);
}

double MountainCar3DScaled::get_reward()
{
...
vector<double> MountainCar3DScaled::get_max_ranges()
{
    ...
    max_ranges.push_back(max_velocity);
    max_ranges.push_back(max_position + max_position);
    max_ranges.push_back(max_velocity + max_velocity);
    max_ranges.push_back(min_position * min_position);
    max_ranges.push_back(max_velocity * max_velocity);
    return max_ranges;
}

vector<double> MountainCar3DScaled::get_min_ranges()
{
    ...
    min_ranges.push_back(min_velocity);
    min_ranges.push_back(min_position + min_position);
    min_ranges.push_back(min_velocity + min_velocity);
    min_ranges.push_back(min_position * max_position);
    min_ranges.push_back(min_velocity * max_velocity);
    return min_ranges;
}

void MountainCar3DScaled::compute_possible_actions()
{
...
MountainCar3DScaled::~MountainCar3DScaled() {
