...
class NNDimReduce(object):
    ...
    def add_validation_data(self, data_vec):
        self.validation_data.append(data_vec)
        return 1
    ...
    def build_dim_reduction(self, dim_max):
        ...
        self.x_validation = np.array(self.validation_data)
        self.cumulative_validation = self.x_validation - self.x_validation
        ...
        for i in range(0,dim_max):
            ...
            encoding_dim = 1
            encoded = Dense(dim_max/2, activation='sigmoid', input_shape=(dim_max,))
            layer = Dense(encoding_dim, activation='sigmoid', input_shape=(dim_max/2,))
            ...
            reconstructed_validation = self.x_validation - self.cumulative_validation
            model_ae.fit(self.x_train, reconstructed,
                            nb_epoch=500,
                            verbose=0,
                            batch_size=2048,
                            shuffle=True,
                            validation_data=(self.x_validation, reconstructed_validation))
                            #validation_split=.2)
            ...
            score = model_ae.evaluate(self.x_validation, reconstructed_validation)
            print('Test Score: ', score)
            self.cumulative_validation += model_ae.predict(self.x_validation) #the reconstructed representation: 4 dim
            #self.compute_mse()
            self.compute_mse_full()
            ...
            #export_model(model_e, 'encoder.model.mtn_car.regular.'+str(i+1))
    ...
    def compute_mse(self):
        ...
        for i in range(0,len(self.x_train)):
            ...
            tot = tot * tot
            all_error += sum(tot)
        print(math.sqrt(all_error/len(self.x_train)))

    def compute_mse_full(self):
        all_error = 0
        tot = 0
        for i in range(0,len(self.x_validation)):
            ele = self.x_validation[i]
            ele2 = self.cumulative_validation[i]
            tot = ele - ele2
            tot = tot * tot
            all_error += sum(tot)
        print(math.sqrt(all_error/len(self.x_validation)))
...
if __name__ == "__main__":
    ...
    with open(filename, 'rb') as f:
        ...
        for row in reader:
            ...
            for ele in row:
                data_row.append(float(ele))
            nn_dim_reduce.add_validation_data(data_row)


    nn_dim_reduce.build_dim_reduction(len(programData[0]))

    print(nn_dim_reduce.transform_down(programData[0], 1))
